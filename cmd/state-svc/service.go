...
type service struct {
	cfg      *config.Instance
	an       *anaSvc.Client
	checker  *deprecation.Checker
	shutdown context.CancelFunc
	server   *server.Server
	ipcSrv   *ipc.Server
}

func NewService(cfg *config.Instance, an *anaSvc.Client, checker *deprecation.Checker, shutdown context.CancelFunc) *service {
	return &service{cfg: cfg, an: an, checker: checker, shutdown: shutdown}
}
...
func (s *service) Start() error {
	...
	var err error
	s.server, err = server.New(s.cfg, s.an, s.checker, s.shutdown)
	if err != nil {
