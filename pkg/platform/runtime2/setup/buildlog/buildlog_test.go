...
func (cm *connectionMock) SendArtifactStartedMessage(a model.Artifact, isCacheHit bool) {
	cm.messages = append(
		cm.messages, message{
...
func (cm *connectionMock) SendArtifactSucceededMessage(a model.Artifact) {
	chksum := "123"
...
func (cm *connectionMock) SendArtifactFailedMessage(a model.Artifact, errMsg string) {
	cm.messages = append(
		cm.messages, message{
...
func TestBuildLog(t *testing.T) {
	...
	artifactMap := map[model.ArtifactID]model.Artifact{
		ids[0]: artifacts[0],
		ids[1]: artifacts[1],
	}
	...
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			var downloads []model.ArtifactDownload
