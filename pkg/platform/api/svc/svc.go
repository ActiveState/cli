...
func NewWithoutRetry(cfg *config.Instance) (*gqlclient.Client, error) {
	port := cfg.GetInt(constants.SvcConfigPort)
	if port <= 0 {
		return nil, locale.NewError("err_svc_no_port", "The State Tool service does not appear to be running (no local port was configured).")
	}
	return gqlclient.NewWithOpts(fmt.Sprintf("http://127.0.0.1:%d/query", port), 0, graphql.WithHTTPClient(http.DefaultClient)), nil
}
