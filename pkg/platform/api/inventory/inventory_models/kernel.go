// Code generated by go-swagger; DO NOT EDIT.

package inventory_models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// Kernel Kernel
//
// The full kernel data model
//
// swagger:model kernel
type Kernel struct {
	KernelAllOf0

	KernelCore
}

// UnmarshalJSON unmarshals this object from a JSON structure
func (m *Kernel) UnmarshalJSON(raw []byte) error {
	// AO0
	var aO0 KernelAllOf0
	if err := swag.ReadJSON(raw, &aO0); err != nil {
		return err
	}
	m.KernelAllOf0 = aO0

	// AO1
	var aO1 KernelCore
	if err := swag.ReadJSON(raw, &aO1); err != nil {
		return err
	}
	m.KernelCore = aO1

	return nil
}

// MarshalJSON marshals this object to a JSON structure
func (m Kernel) MarshalJSON() ([]byte, error) {
	_parts := make([][]byte, 0, 2)

	aO0, err := swag.WriteJSON(m.KernelAllOf0)
	if err != nil {
		return nil, err
	}
	_parts = append(_parts, aO0)

	aO1, err := swag.WriteJSON(m.KernelCore)
	if err != nil {
		return nil, err
	}
	_parts = append(_parts, aO1)
	return swag.ConcatJSON(_parts...), nil
}

// Validate validates this kernel
func (m *Kernel) Validate(formats strfmt.Registry) error {
	var res []error

	// validation for a type composition with KernelAllOf0
	if err := m.KernelAllOf0.Validate(formats); err != nil {
		res = append(res, err)
	}
	// validation for a type composition with KernelCore
	if err := m.KernelCore.Validate(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

// ContextValidate validate this kernel based on the context it is used
func (m *Kernel) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	// validation for a type composition with KernelAllOf0
	if err := m.KernelAllOf0.ContextValidate(ctx, formats); err != nil {
		res = append(res, err)
	}
	// validation for a type composition with KernelCore
	if err := m.KernelCore.ContextValidate(ctx, formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

// MarshalBinary interface implementation
func (m *Kernel) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *Kernel) UnmarshalBinary(b []byte) error {
	var res Kernel
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
