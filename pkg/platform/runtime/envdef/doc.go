...
//
// Assuming the runtime is installed to a directory `/home/user/.cache/installdir`,
// the following definition asks to set the PATH variables to
// `/home/user/.cache/installdir/bin:/home/user/.cache/installdir/usr/bin` and
// `LD_LIBRARY_PATH` to
// `/home/user/.cache/installdir/lib`
// The set `inherit` flag on the `PATH` variable ensures that the `PATH` value
...
// inside the artifact tarball that needs to be unpacked to `/home/user/.cache/installdir`
//
...
// the `PATH` to `/home/user/.cache/installdir/bin:/home/user/.cache/installdir/usr/local/bin:/home/user/.cache/installdir/usr/bin`.
// Note, that duplicate values are filtered out.
// Likewise the `LD_LIBRARY_PATH` will end up as `/home/user/.cache/installdir/lib:/home/user/.cache/installdir/lib64`
//
// In this example, the values were joined by prepending the second definition
// to the first.
// Other join strategies are `append` and `disallowed`.
//
// The `disallowed` join strategy can be used if a variable should have only ONE
// value, and this value needs to be the same or undefined between all artifacts
// that depend on it.
//
// ## Usage
//
