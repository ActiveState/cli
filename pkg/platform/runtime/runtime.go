...
func (r *Runtime) Update(auth *authentication.Auth, msgHandler *events.RuntimeEventHandler, sockPath string) error {
	...
	go func() {
		...
		if err := setup.New(r.target, prod, auth, r.analytics).Update(sockPath); err != nil {
