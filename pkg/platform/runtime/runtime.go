...
func (r *Runtime) Update(auth *authentication.Auth, msgHandler *events.RuntimeEventHandler, projPath, sockPath string) error {
	...
	go func() {
		...
		if err := setup.New(r.target, prod, auth, r.analytics).Update(projPath, sockPath); err != nil {
