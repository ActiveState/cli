...
func (suite *GitTestSuite) TestCloneProjectRepo() {
	...
	repo := NewRepo()
	err = repo.CloneProject("test-owner", "test-project", targetDir, outputhelper.NewCatcher(), analyMock.New())
...
func (suite *GitTestSuite) TestEnsureCorrectProject() {
	err := ensureCorrectProject("test-owner", "test-project", filepath.Join(suite.dir, constants.ConfigFileName), "test-repo", outputhelper.NewCatcher(), analyMock.New())
...
func (suite *GitTestSuite) TestEnsureCorrectProject_Mistmatch() {
	...
	err := ensureCorrectProject(owner, name, projectPath, "test-repo", actualCatcher, analyMock.New())
	suite.NoError(err)
