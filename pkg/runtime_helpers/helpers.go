...
func NeedsUpdate(proj *project.Project, overrideCommitID *strfmt.UUID, info *checkoutinfo.CheckoutInfo) (bool, error) {
	rt, err := FromProject(proj)
	if err != nil {
		return false, errs.Wrap(err, "Could not obtain runtime")
	}

	hash, err := Hash(proj, overrideCommitID, info)
	if err != nil {
...
func Hash(proj *project.Project, overrideCommitID *strfmt.UUID, info *checkoutinfo.CheckoutInfo) (string, error) {
	var err error
