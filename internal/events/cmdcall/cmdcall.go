...
func New(p primeable, cmdList string, nonInteractive bool) *CmdCall {
	return &CmdCall{
		out:       p.Output(),
		proj:      p.Project(),
		subshell:  p.Subshell(),
		cmdList:   cmdList,
		p:         p,
		scriptrun: scriptrun.New(p.Auth(), p.Output(), p.Subshell(), p.Project(), p.Config(), p.Analytics(), p.SvcModel(), nonInteractive),
	}
}
