...
scripts:
  ...
  `)
	...
	scriptRun := New(authentication.LegacyGet(), outputhelper.NewCatcher(), subshell.New(cfg), proj, cfg, analyMock.New())
...
`)
	...
	scriptRun := New(authentication.LegacyGet(), out, subshell.New(cfg), proj, cfg, analyMock.New())
