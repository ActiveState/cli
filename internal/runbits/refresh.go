...
func RefreshRuntime(auth *authentication.Auth, out output.Outputer, an analytics.Dispatcher, proj *project.Project, cachePath string, commitID strfmt.UUID, changed bool, trigger target.Trigger, svcm *model.SvcModel, nonInteractive bool) error {
	var rtMessages *events.RuntimeEventHandler
	var err error
	if nonInteractive {
		rtMessages, err = DefaultNonInteractiveRuntimeEventHandler(out)
	} else {
