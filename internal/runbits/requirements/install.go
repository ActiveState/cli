...
func (r *RequirementOperation) Install(ts *time.Time, requirements []*Requirement, namespaces []*model.Namespace) (rerr error) {
	...
	// Solve the runtime.
	rt, rtCommit, err := runtime.Solve(r.Auth, r.Output, r.Analytics, r.Project, &stagedCommitID, trigger, r.SvcModel, r.Config, runtime.OptNone)
	if err != nil {
...
func (r *RequirementOperation) InstallPlatform(requirements []*Requirement) (rerr error) {
	...
	// Solve the runtime.
	rt, rtCommit, err := runtime.Solve(r.Auth, r.Output, r.Analytics, r.Project, &stagedCommitID, target.TriggerPlatform, r.SvcModel, r.Config, runtime.OptNone)
	if err != nil {
		return errs.Wrap(err, "Could not solve runtime")
	}
