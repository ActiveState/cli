...
func UpdateAndReload(prime primeable, script *buildscript.BuildScript, oldCommit *buildplanner.Commit, commitMsg string, trigger trigger.Trigger) error {
	...
	if !cfg.GetBool(constants.AsyncRuntimeConfig) {
		...
		if err != nil {
			if !isBuildError(err) {
				...
				if err2 := updateBuildScript(prime, newCommit.BuildScript()); err2 != nil {
	...
	// Update commit ID
	if err := updateBuildScript(prime, newCommit.BuildScript()); err != nil {
		return locale.WrapError(err, "err_package_update_commit_id")
	}
	return nil
}

func updateBuildScript(prime primeable, script *buildscript.BuildScript) error {
	err := prime.CheckoutInfo().UpdateBuildScript(script)
	if err != nil {
