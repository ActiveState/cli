...
func (cf *ConversionFlow) StartIfNecessary() (bool, error) {
	...
	cf.analytics.Event(analytics.CatPpmConversion, "run")
	r, err := cf.runSurvey()
	if err != nil {
		cf.analytics.EventWithLabel(analytics.CatPpmConversion, "error", errs.Join(err, " :: ").Error())
		return true, locale.WrapError(err, "ppm_conversion_survey_error", "Conversion flow failed.")
	}

	if r != accepted {
		cf.analytics.EventWithLabel(analytics.CatPpmConversion, "completed", r.String())
...
func (cf *ConversionFlow) runSurvey() (conversionResult, error) {
	...
	cf.analytics.EventWithLabel(analytics.CatPpmConversion, "selection", eventChoices[choice])
