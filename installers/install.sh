...
fetchArtifact () {
  ...
  $FETCH $TMPDIR/$STATEJSON $STATEURL$STATEJSON || return 1
  ...
  # Fetch it.
  $FETCH $TMPDIR/$STATEPKG ${STATEURL}${VERSION}/${STATEPKG} || return 1
  ...
  if [ $OS = "windows" ]; then
    ...
    tar -xzf $TMPDIR/$STATEPKG -C $TMPDIR || return 1
  fi
